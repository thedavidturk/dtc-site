...
const ACCENT = new THREE.Color("#101731");
...
function OrbitRing() {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial color="#101731" size={0.04} transparent opacity={0.7} sizeAttenuation />
    </points>
  );
}
    ...
    <group ref={ref}>
      {[0, 1, 2].map((i) => (
        <mesh key={i} position={[0, -0.5 + i * 0.5, 0]} rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshBasicMaterial color="#101731" transparent opacity={0.12} side={THREE.DoubleSide} />
        </mesh>
      ))}
    </group>
  );
