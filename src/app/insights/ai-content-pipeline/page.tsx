  ...
  return (
    <article className="bg-sand min-h-screen">
      ...
      <section className="relative min-h-[60vh] md:min-h-[70vh] flex items-end overflow-hidden">
        ...
        <div className="absolute top-1/4 left-[20%] w-32 h-32 border border-ink/[0.06] rounded-full animate-pulse" />
        <div className="absolute top-1/3 right-1/4 w-20 h-20 border border-ink/10 rounded-xl rotate-12" />
        <div className="absolute bottom-1/3 left-1/3 w-3 h-3 bg-ink/20 rounded-full" />
        ...
        <div className="section-container relative z-10 pb-16 md:pb-24 pt-32">
          ...
          <m.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.45, ease: [0.22, 1, 0.36, 1] }} className="font-body text-lg md:text-xl text-ink/70 max-w-2xl">
          ...
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.6 }} className="flex items-center gap-4 mt-6">
            <span className="font-mono text-xs text-ink/50 tracking-wide">David Turk</span>
            <span className="w-1 h-1 rounded-full bg-ink/30" />
            <span className="font-mono text-xs text-ink/50 tracking-wide">9 min read</span>
          </m.div>

          <m.div initial={{ width: 0 }} animate={{ width: "6rem" }} transition={{ duration: 0.8, delay: 0.7, ease: [0.22, 1, 0.36, 1] }} className="h-px bg-gradient-to-r from-marigold to-blue-400 mt-8" />
        </div>
      </section>
      ...
      <section className="section-container section-padding">
        <m.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="max-w-3xl mx-auto">
          ...
          <m.blockquote variants={fadeUp} className="pl-6 border-l-2 border-marigold text-ink italic text-xl my-8">
      ...
      <section className="section-container section-padding">
        <m.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} className="mb-12">
          <p className="font-mono text-sm text-marigold tracking-widest uppercase mb-4">Continue Reading</p>
        ...
        <m.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {moreArticles.map((article) => (
            <m.div key={article.href} variants={staggerItem}>
              <Link href={article.href} className="group block rounded-2xl border border-ink/5 bg-ink/[0.02] overflow-hidden hover:border-ink/15 hover:-translate-y-1 transition-all duration-500">
                ...
                <div className="p-6">
                  <span className="font-mono text-xs text-marigold uppercase tracking-wider">{article.category}</span>
                  <h3 className="font-headline font-bold text-lg text-ink mt-2 leading-snug group-hover:text-ink transition-colors duration-300">{article.title}</h3>
