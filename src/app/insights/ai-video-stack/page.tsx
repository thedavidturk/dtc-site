...
export default function AiVideoStackArticle() {
  return (
    <main className="relative min-h-screen bg-espresso text-bone-white overflow-hidden">
      ...
      <Link
        href="/"
        className="fixed top-6 left-6 z-50 text-sm text-bone-white/60 hover:text-bone-white transition-colors"
      >
        &larr; Back to home
      </Link>
        ...
        <div className="relative max-w-3xl mx-auto">
          <m.div initial="hidden" animate="visible" variants={staggerContainer}>
            <m.div variants={staggerItem} className="mb-6">
              <span className="inline-block px-3 py-1 text-xs font-normal tracking-wider text-bone-white/80 border border-black/20 rounded-full">
                AI + VIDEO
              </span>
            </m.div>
            <m.h1
            ...
            <m.p
              variants={staggerItem}
              className="text-lg md:text-xl text-bone-white/70 leading-relaxed"
            >
            ...
            <m.div
              variants={staggerItem}
              className="mt-8 flex items-center gap-4 text-sm text-bone-white/50"
            >
              <span>David Turk</span>
              <span className="w-1 h-1 rounded-full bg-black/30" />
              <span>8 min read</span>
            </m.div>
            <m.div variants={staggerItem} className="mt-10 h-px bg-gradient-to-r from-black/40 to-transparent" />
          </m.div>
        </div>
      </section>
      ...
      <article className="relative px-6 pb-24">
        <div className="max-w-2xl mx-auto space-y-10">
          ...
          <m.blockquote
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-80px" }}
            className="border-l-2 border-black/40 pl-6 py-2 my-12"
          >
            <p className="text-xl md:text-2xl font-normal leading-relaxed text-bone-white/90">
          ...
          <m.div
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-80px" }}
            className="h-px bg-black/10 my-16"
          />
          ...
          <div className="space-y-8">
            <m.h3
              ...
              whileInView="visible"
              viewport={{ once: true, margin: "-80px" }}
              className="text-sm font-normal tracking-wider text-bone-white/50"
            >
              MORE PERSPECTIVES
            </m.h3>
              ...
              {moreArticles.map((article) => (
                ...
                >
                  <Link href={article.href} className="group block">
                    <div
                      className={`relative overflow-hidden rounded-none border border-black/10 bg-gradient-to-br ${article.gradient} p-6 transition-transform group-hover:scale-[1.02]`}
                    >
                      <span className="text-xs font-normal tracking-wider text-bone-white/60">
                        {article.category}
                      </span>
                      <h4 className="mt-2 text-lg font-light text-bone-white">
                        {article.title}
                      </h4>
                    </div>
                  </Link>
